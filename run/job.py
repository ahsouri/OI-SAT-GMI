...
# in case of TEMPO, we have to do everything over a loop of segments
if sensor == "TEMPO":
   for segment in range(1,14):
       ...
       oisatgmi_obj.read_data(ctm_name, Path(ctm_dir), gas, ctm_freq, sensor+'_'+gas,
                       ...
                       trop=troposphere_only, num_job=int(num_job), mcip_dir=Path(mcip_dir), segment_number = segment)
       ...
       oisatgmi_obj.reporting(gas + '_' + str(segment_number) + str(year) + f"{month:02}", gas, output_pdf_dir)
       oisatgmi_obj.write_to_nc(gas + '_' + str(segment_number) + str(year) + f"{month:02}", output_nc_dir)
